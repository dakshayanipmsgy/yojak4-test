...
function pack_print_v3_extract_body(string $html): string
{
...
function pack_print_v3_generated_templates(array $pack, array $contractor): array
{
    $templates = [];
    foreach ($pack['generatedTemplates'] ?? [] as $tpl) {
        $storedPath = trim((string)($tpl['storedPath'] ?? ''));
        if ($storedPath === '') {
            continue;
        }
        $relative = $storedPath[0] === '/' ? $storedPath : '/' . $storedPath;
        $fullPath = realpath(PUBLIC_PATH . $relative);
        if ($fullPath === false || !file_exists($fullPath) || !is_path_within($fullPath, PUBLIC_PATH)) {
            continue;
        }
        ...
        $templates[] = [
            'name' => $tpl['name'] ?? 'Template',
            'html' => pack_print_v3_extract_body($content),
        ];
    }
    ...
    $fallback = [];
    foreach (pack_template_payloads($pack, $contractor) as $tpl) {
        $fallback[] = [
            'name' => $tpl['name'] ?? 'Template',
            'html' => '<pre>' . htmlspecialchars($tpl['body'] ?? '', ENT_QUOTES, 'UTF-8') . '</pre>',
        ];
...
function pack_print_full_v3_html(array $pack, array $contractor, array $options = [], array $annexureTemplates = []): string
{
    $pack = pack_apply_schema_defaults($pack);
    $contractor = normalize_contractor_profile($contractor);
    $options = array_merge([
    ...
    $autoprint = !empty($options['autoprint']);
    $printedAt = now_kolkata()->format('d M Y, h:i A');
    $annexureTemplates = $annexureTemplates ?: load_pack_annexures($pack['yojId'], $pack['packId'], detect_pack_context($pack['packId']));
    $attachments = pack_attachment_map($pack, contractor_vault_index($pack['yojId']));

    $render_checklist = static function () use ($pack, $attachments): string {
        $checklist = $pack['checklist'] ?? [];
        if (!$checklist) {
            return '<p class="muted">No checklist data available.</p>';
        }
        $grouped = [];
        foreach ($checklist as $item) {
            $group = $item['category'] ?? 'Other';
        ...
        foreach ($grouped as $group => $items) {
            ...
            foreach ($items as $item) {
                ...
                $html .= '<td><strong>' . htmlspecialchars($item['title'] ?? '', ENT_QUOTES, 'UTF-8') . '</strong><div class="muted">' . htmlspecialchars($item['description'] ?? '', ENT_QUOTES, 'UTF-8') . '</div></td>';
                $html .= '<td>' . (!empty($item['required']) ? 'Required' : 'Optional') . '</td>';
                $html .= '<td>' . htmlspecialchars(ucfirst($item['status'] ?? 'pending'), ENT_QUOTES, 'UTF-8') . '</td>';
                $html .= '<td>' . ($notes !== '' ? htmlspecialchars($notes, ENT_QUOTES, 'UTF-8') : '<span class="muted">-</span>') . '</td>';
                if ($attach) {
                    $title = htmlspecialchars($attach['title'] ?? 'Vault document', ENT_QUOTES, 'UTF-8');
                    $html .= '<td><div class="pill">' . $title . ' (' . htmlspecialchars($attach['fileId'] ?? '', ENT_QUOTES, 'UTF-8') . ')</div></td>';
                } else {
                    $html .= '<td><span class="muted">Not mapped</span></td>';
                }
                $html .= '</tr>';
    ...
    $render_annexures = static function () use ($pack, $annexureTemplates, $contractor): string {
        $annexures = $pack['annexures'] ?? [];
        $formats = $pack['formats'] ?? [];
        $catalog = pack_field_meta_catalog($pack, $annexureTemplates);
        $html = '';
        ...
        if ($annexureTemplates) {
            $html .= '<h3>Generated Annexure Templates</h3>';
            foreach ($annexureTemplates as $idx => $tpl) {
                $bodyHtml = pack_render_annexure_body_html($tpl, $pack, $contractor, $catalog, true);
                $tablesHtml = pack_render_template_tables_html($tpl, $pack, $contractor, $catalog, true);
                $html .= '<div class="template-block' . ($idx > 0 ? ' page-break' : '') . '">';
                $html .= '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;">';
                $html .= '<div><div class="muted">' . htmlspecialchars($tpl['annexureCode'] ?? 'Annexure', ENT_QUOTES, 'UTF-8') . '</div><h4 style="margin:4px 0 6px 0;">' . htmlspecialchars($tpl['title'] ?? 'Annexure', ENT_QUOTES, 'UTF-8') . '</h4></div>';
                $html .= '<span class="pill">' . htmlspecialchars(ucwords(str_replace('_', ' ', $tpl['type'] ?? 'other')), ENT_QUOTES, 'UTF-8') . '</span>';
                $html .= '</div>';
                $html .= '<div class="template-body">' . $bodyHtml . '</div>';
                $html .= $tablesHtml;
        ...
        return $html !== '' ? $html : '<p class="muted">No annexures or formats available.</p>';
    };

    $render_templates = static function (array $templates): string {
        $html = '';
        foreach ($templates as $idx => $tpl) {
            $html .= '<div class="template-block' . ($idx > 0 ? ' page-break' : '') . '">';
            $html .= '<h4 style="margin:0 0 6px 0;">' . htmlspecialchars($tpl['name'] ?? 'Template', ENT_QUOTES, 'UTF-8') . '</h4>';
            $html .= '<div class="template-body">' . ($tpl['html'] ?? '') . '</div>';
            $html .= '</div>';
        }
        return $html !== '' ? $html : '<p class="muted">No templates available.</p>';
    };

    $render_pack_index = static function () use ($pack, $printedAt): string {
        $stats = pack_stats($pack);
        $annexureList = $pack['annexureList'] ?? ($pack['annexures'] ?? []);
        $templateList = array_map(static function (array $tpl): string {
            return (string)($tpl['name'] ?? 'Template');
        ...
        $html .= '<div><h3>Tender Overview</h3><ul>';
        $html .= '<li>Title: ' . htmlspecialchars($pack['tenderTitle'] ?? $pack['title'] ?? 'Tender Pack', ENT_QUOTES, 'UTF-8') . '</li>';
        $html .= '<li>Pack ID: ' . htmlspecialchars($pack['packId'] ?? '', ENT_QUOTES, 'UTF-8') . '</li>';
        $html .= '<li>Tender No: ' . htmlspecialchars($pack['tenderNumber'] ?? '', ENT_QUOTES, 'UTF-8') . '</li>';
        $html .= '<li>Department: ' . htmlspecialchars($pack['departmentName'] ?? ($pack['deptName'] ?? ''), ENT_QUOTES, 'UTF-8') . '</li>';
        $html .= '<li>Submission: ' . htmlspecialchars($pack['submissionDeadline'] ?? ($pack['dates']['submission'] ?? ''), ENT_QUOTES, 'UTF-8') . '</li>';
        $html .= '<li>Opening: ' . htmlspecialchars($pack['openingDate'] ?? ($pack['dates']['opening'] ?? ''), ENT_QUOTES, 'UTF-8') . '</li>';
        $html .= '<li>Generated: ' . htmlspecialchars($printedAt, ENT_QUOTES, 'UTF-8') . '</li>';
        $html .= '</ul></div>';
        $html .= '<div><h3>Checklist Summary</h3><ul>';
        $html .= '<li>Total items: ' . $stats['totalItems'] . '</li>';
        $html .= '<li>Required: ' . $stats['requiredItems'] . '</li>';
        $html .= '<li>Done required: ' . $stats['doneRequired'] . '</li>';
        $html .= '<li>Pending required: ' . $stats['pendingRequired'] . '</li>';
        $html .= '</ul>';
        $html .= '<h3>Included Annexures</h3><ul>';
        if ($annexureList) {
            foreach ($annexureList as $annex) {
                $label = is_array($annex) ? ($annex['title'] ?? ($annex['name'] ?? 'Annexure')) : (string)$annex;
                $html .= '<li>' . htmlspecialchars($label, ENT_QUOTES, 'UTF-8') . '</li>';
        ...
        $html .= '</ul><h3>Included Templates</h3><ul>';
        if ($templateList) {
            foreach ($templateList as $tplName) {
                $html .= '<li>' . htmlspecialchars($tplName, ENT_QUOTES, 'UTF-8') . '</li>';
            }
    ...
    $templates = pack_print_v3_generated_templates($pack, $contractor);
    $categories = [
        'covering-letter' => ['title' => 'Covering Letter', 'items' => []],
        'declarations' => ['title' => 'Declarations', 'items' => []],
        'undertakings' => ['title' => 'Undertakings', 'items' => []],
        'company-profile' => ['title' => 'Company Profile', 'items' => []],
        'experience' => ['title' => 'Experience / Manpower / Equipment', 'items' => []],
        'technical-bid' => ['title' => 'Technical Bid', 'items' => []],
        'commercial-bid' => ['title' => 'Commercial Bid', 'items' => []],
        'templates' => ['title' => 'Templates', 'items' => []],
    ];
    foreach ($templates as $tpl) {
        $name = strtolower((string)($tpl['name'] ?? ''));
        $target = 'templates';
        if (str_contains($name, 'covering letter') || str_contains($name, 'cover letter')) {
            $target = 'covering-letter';
        } elseif (str_contains($name, 'declaration')) {
            $target = 'declarations';
        } elseif (str_contains($name, 'undertaking')) {
            $target = 'undertakings';
        } elseif (str_contains($name, 'company profile')) {
            $target = 'company-profile';
        } elseif (str_contains($name, 'experience') || str_contains($name, 'manpower') || str_contains($name, 'equipment')) {
            $target = 'experience';
        } elseif (str_contains($name, 'technical')) {
            $target = 'technical-bid';
        } elseif (str_contains($name, 'commercial') || str_contains($name, 'financial') || str_contains($name, 'price')) {
            $target = 'commercial-bid';
        }
        $categories[$target]['items'][] = $tpl;
    }

    $sections = [];
    $sections[] = ['id' => 'sec-pack-index', 'title' => 'Pack Index', 'html' => $render_pack_index()];
    if (!empty($pack['checklist'])) {
        $sections[] = ['id' => 'sec-checklist', 'title' => 'Checklist', 'html' => $render_checklist()];
    }
    if (!empty($pack['annexures']) || !empty($pack['formats']) || $annexureTemplates) {
        $sections[] = ['id' => 'sec-annexures', 'title' => 'Annexures & Formats', 'html' => $render_annexures()];
    }
    foreach ($categories as $key => $category) {
        if ($category['items']) {
            $sections[] = ['id' => 'sec-' . $key, 'title' => $category['title'], 'html' => $render_templates($category['items'])];
        }
    }

    $tocItems = '';
    foreach ($sections as $section) {
        $tocItems .= '<li><a href="#' . htmlspecialchars($section['id'], ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($section['title'], ENT_QUOTES, 'UTF-8') . '</a></li>';
    }

    $content = '';
    foreach ($sections as $index => $section) {
        $content .= '<section id="' . htmlspecialchars($section['id'], ENT_QUOTES, 'UTF-8') . '" class="section' . ($index > 0 ? ' page-break' : '') . '">';
    ...
    h2{font-size:1.3em;margin:18px 0 8px;break-after:avoid;}
    h3{font-size:1.1em;margin:12px 0 6px;break-after:avoid;}
    h4{font-size:1.05em;margin:10px 0 6px;break-after:avoid;}
    .muted{color:#555;}
    .header-block{padding-bottom:12px;border-bottom:1px solid #333;margin-bottom:12px;}
    .header-meta{display:grid;gap:4px;font-size:0.95em;}
    .toc{list-style:none;padding:0;margin:0;}
    .toc li{display:flex;gap:8px;margin:4px 0;align-items:baseline;}
    .toc a{color:#000;text-decoration:none;flex:1;}
    .toc a::after{content: leader(".") " " target-counter(attr(href), page);}
    @supports not (content: target-counter(attr(href), page)) {
        .toc a::after{content:"";}
    }
    .section{margin-top:12px;break-inside:avoid-page;}
    .page-break{break-before:page;page-break-before:always;}
    table{width:100%;border-collapse:collapse;table-layout:fixed;}
    th,td{padding:6px 8px;border:1px solid #333;vertical-align:top;word-break:break-word;overflow-wrap:anywhere;}
    th{background:#f3f3f3;text-transform:uppercase;font-size:0.8em;letter-spacing:0.04em;}
    .pill{display:inline-block;border:1px solid #333;border-radius:999px;padding:2px 8px;font-size:0.85em;}
    .template-block{margin-top:12px;padding-top:8px;border-top:1px solid #333;break-inside:avoid;}
    .template-body{white-space:pre-wrap;line-height:1.6;}
    .index-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:12px;}
    .index-grid ul{margin:0;padding-left:18px;}
    .page-footer{position:fixed;bottom:6mm;left:0;right:0;text-align:center;font-size:0.85em;}
    .page-footer::after{content:"Page " counter(page);}
    @supports (content: counter(pages)) {
    ...
    $autoPrintScript = '';
    if ($autoprint) {
        $autoPrintScript = '<script>(()=>{window.addEventListener("load",()=>setTimeout(()=>window.print(),300));})();</script>';
    }

    $html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pack '
        . htmlspecialchars($pack['packId'] ?? 'Pack', ENT_QUOTES, 'UTF-8') . '</title>' . $styles
        . '</head><body class="density-' . htmlspecialchars($density, ENT_QUOTES, 'UTF-8') . '">'
        . '<div class="header-block"><h1>' . htmlspecialchars($pack['tenderTitle'] ?? ($pack['title'] ?? 'Tender Pack'), ENT_QUOTES, 'UTF-8') . '</h1>'
        . '<div class="header-meta"><div>Pack ID: ' . htmlspecialchars($pack['packId'] ?? '', ENT_QUOTES, 'UTF-8') . '</div>'
        . '<div>Tender No: ' . htmlspecialchars($pack['tenderNumber'] ?? '', ENT_QUOTES, 'UTF-8') . '</div>'
        . '<div>Printed on: ' . htmlspecialchars($printedAt, ENT_QUOTES, 'UTF-8') . '</div></div></div>'
        . '<section class="section" id="sec-contents"><h2>Index / Contents</h2><ul class="toc">' . $tocItems . '</ul></section>'
        . $content

...
safe_page(function () {
    ...
    $header = $showHeader ? '<div style="padding:12px;border-bottom:1px solid #30363d;display:flex;align-items:center;gap:12px;">'
        ...
        . '<div style="color:#8b949e;font-size:12px;">' . sanitize($deptId) . '</div></div></div>' : '';

    $footer = $showFooter ? '<div style="padding:12px;border-top:1px solid #30363d;margin-top:16px;color:#8b949e;font-size:12px;">'
        . 'Generated by ' . sanitize($user['username'] ?? '') . ' on ' . sanitize(now_kolkata()->format('d M Y H:i')) . '</div>' : '';

    $renderedHtml = '<div style="background:#0d1117;color:#e6edf3;font-family:Segoe UI,system-ui,sans-serif;">'
        . $header

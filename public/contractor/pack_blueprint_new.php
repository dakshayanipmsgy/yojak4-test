    ...
    render_layout($title, function () use ($globalTemplates, $myTemplates) {
        ...
        <div class="card">
            <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap;">
                <div>
                    <h2 style="margin:0;">Create Pack Blueprint</h2>
                    <p class="muted" style="margin:4px 0 0;">Build a reusable pack with checklists, templates, and required uploads.</p>
                </div>
                <a class="btn secondary" href="/contractor/packs_library.php?tab=mine">Back to Blueprints</a>
            </div>
        </div>

        <form method="post" action="/contractor/pack_blueprint_create.php" style="margin-top:12px;">
            <input type="hidden" name="csrf_token" value="<?= sanitize(csrf_token()); ?>">
            <div class="card" style="display:grid; gap:12px;">
                <label class="field">
                    <span>Title</span>
                    <input type="text" name="title" required placeholder="Standard EPC Tender Pack">
                </label>
                <label class="field">
                    <span>Description</span>
                    <textarea name="description" rows="2" placeholder="What this pack is for"></textarea>
                </label>
                <div>
                    <strong>Items</strong>
            ...
            <div style="display:flex; gap:10px; margin-top:12px;">
                <button class="btn" type="submit">Save Blueprint</button>

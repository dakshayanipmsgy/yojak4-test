...
safe_page(function () {
    ...
    $entries = array_filter(guide_index_entries(), function ($entry) {
    ...
    $selectedId = guide_sanitize_id((string)($_GET['id'] ?? ''));
    $selected = null;
    ...
    $title = get_app_config()['appName'] . ' | Contractor Guide';
    ...
    render_layout($title, function () use ($sections, $selected, $selectedId, $user) {
        ...
        <div class="card" style="margin-bottom:16px;">
            <div class="guide-meta">
                <div>
                    <h2 style="margin:0;">Contractor Guide</h2>
                    <p class="muted" style="margin:6px 0 0;">Learn YOJAK workflows step-by-step. Welcome, <?= sanitize($user['displayName'] ?? $user['username']); ?>.</p>
                </div>
        ...
        <div class="guide-layout">
            <aside class="guide-sidebar guide-desktop-only">
                <div class="guide-list" id="guide-list">
                    <?php foreach ($sections as $section): ?>
                        <a href="/contractor/guide.php?id=<?= sanitize($section['id'] ?? ''); ?>"
            ...
            <section>
                <?php if (!$selected): ?>
                    <div class="card guide-empty">
                        <p class="muted">No published guide sections yet. Please check back soon.</p>
                    </div>
                <?php else: ?>
                    <div class="card" style="margin-bottom:16px;">
                                ...
                                <?php elseif ($type === 'do_dont'): ?>
                                    <div class="guide-do-dont">
                                        ...
                                        <div class="card" style="border-color:#fecaca; background:#fff1f2;">
                                            <strong>YOJAK will NOT do</strong>
                                            <ul>
            ...
            (() => {
                ...
                if (select) {
                    select.addEventListener('change', () => {
                        ...
                        if (id) {
                            window.location.href = `/contractor/guide.php?id=${encodeURIComponent(id)}`;
                        }
                    });
                }

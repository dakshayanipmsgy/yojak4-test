...
safe_page(function () {
    ...
    if ($query !== '') {
        $templates = array_values(array_filter($templates, static function ($tpl) use ($query) {
            $haystack = strtolower((string)($tpl['title'] ?? ($tpl['name'] ?? '')));
    ...
    $history = create_docs_list_generated(contractor_generated_docs_path($yojId));
    ...
    render_layout($title, function () use ($templates, $history, $query, $category, $categories) {
        ...
        <div class="card">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                ...
                <a class="btn secondary" href="/contractor/templates.php">Manage Templates</a>
            </div>
            <form method="get" style="margin-top:12px;display:grid;gap:12px;grid-template-columns:minmax(0,2fr) minmax(0,1fr);">
                ...
                <div style="grid-column:1 / -1;display:flex;gap:8px;flex-wrap:wrap;">
                    ...
                    <a class="btn secondary" href="/contractor/create_docs.php">Reset</a>
                </div>
            </form>
        </div>
            ...
            <?php foreach ($templates as $tpl): ?>
                ...
                <div class="card" style="display:grid;gap:10px;">
                    <div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;">
                        ...
                        <a class="btn" href="/contractor/create_doc_new.php?templateId=<?= urlencode($templateId); ?>&scope=<?= urlencode((string)($tpl['scope'] ?? '')); ?>">Create Doc</a>
                    </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($history as $doc): ?>
                            <tr>
                                ...
                                <td><a class="btn secondary" href="/contractor/create_doc_view.php?docId=<?= urlencode($doc['docId'] ?? ''); ?>">View</a></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>

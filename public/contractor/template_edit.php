...
safe_page(function () {
    ...
    render_layout($title, function () use ($template, $missingFields, $fieldCatalog, $tableCatalog, $wasMigrated) {
        $grouped = [
            'Contractor' => [],
            'Tender' => [],
            'Case/Scheme' => [],
        ...
        <div class="card">
            <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap;">
                <div>
                    ...
                    <p class="muted" style="margin:4px 0 0;">Update your template content and placeholders.</p>
                </div>
                <div style="display:flex; gap:8px; flex-wrap:wrap;">
                    ...
                    <form method="post" action="/contractor/template_delete.php" onsubmit="return confirm('Delete this template?');">
                        ...
                        <input type="hidden" name="tplId" value="<?= sanitize($template['id'] ?? ($template['tplId'] ?? '')); ?>">
        ...
        <form method="post" action="/contractor/template_update.php" style="margin-top:12px;">
            <input type="hidden" name="csrf_token" value="<?= sanitize(csrf_token()); ?>">
            <input type="hidden" name="tplId" value="<?= sanitize($template['id'] ?? ($template['tplId'] ?? '')); ?>">
            <div style="display:grid; gap:12px; grid-template-columns: minmax(0, 2.2fr) minmax(0, 1fr);">
                <div class="card" style="display:grid; gap:12px;">
                    <div style="display:grid; gap:10px; grid-template-columns:repeat(auto-fit,minmax(220px,1fr));">
                        <label class="field">
                            <span>Title</span>
                            <input type="text" name="title" required value="<?= sanitize($template['title'] ?? ($template['name'] ?? '')); ?>">
                        </label>
                        <label class="field">
                            <span>Category</span>
                    ...
                    <label class="field">
                        ...
                        <textarea id="template-body" name="body" rows="14" required><?= sanitize($template['body'] ?? ''); ?></textarea>
                    </label>

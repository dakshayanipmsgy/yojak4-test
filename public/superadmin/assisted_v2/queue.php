        ...
        <div class="card" style="display:grid;gap:10px;">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
                <?php foreach ($counts as $tab => $count): ?>
                    <a class="pill" href="/superadmin/assisted_v2/queue.php?status=<?= sanitize($tab); ?>" style="<?= $status === $tab ? 'border-color:#1f6feb;color:#9cc4ff;' : ''; ?>">
                        <?= sanitize(ucwords(str_replace('_',' ', $tab)) . ' (' . $count . ')'); ?>
                    </a>
                <?php endforeach; ?>
                <a class="pill" href="/superadmin/assisted_v2/queue.php" style="<?= $status === '' ? 'border-color:#1f6feb;color:#9cc4ff;' : ''; ?>"><?= sanitize('All'); ?></a>
            </div>
        </div>
        <div class="card">
                    ...
                    <tbody>
                        ...
                        <?php foreach ($filtered as $req): ?>
                            <tr>
                                ...
                                <td><a class="btn secondary" href="/superadmin/assisted_v2/process.php?reqId=<?= sanitize(urlencode($req['reqId'] ?? '')); ?>"><?= sanitize('Open'); ?></a></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>

    ...
    render_layout($title, function () use ($request, $tender, $templatesIndex, $promptText, $draftSummary, $draftPayload, $draftStats, $draftWarnings, $previewBundle) {
        ...
        <div class="card" style="display:grid;gap:12px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
                ...
                <a class="btn secondary" href="/superadmin/assisted_v2/queue.php"><?= sanitize('Back to queue'); ?></a>
            </div>
        </div>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:12px;margin-top:12px;">
            ...
            <div class="card" style="display:grid;gap:12px;">
                ...
                <form method="post" action="/superadmin/assisted_v2/apply_template.php" style="display:grid;gap:8px;">
            ...
            <div class="card" style="display:grid;gap:12px;">
                ...
                <form method="post" action="/superadmin/assisted_v2/paste_validate.php" style="display:grid;gap:8px;">
                    <input type="hidden" name="csrf_token" value="<?= sanitize(csrf_token()); ?>">
                    <input type="hidden" name="reqId" value="<?= sanitize($request['reqId'] ?? ''); ?>">
                    <label class="field">
                ...
                <?php if ($draftSummary): ?>
                    ...
                    <?php if ($previewBundle): ?>
                        ...
                        <?php endif; ?>
                    <?php endif; ?>
                    <form method="post" action="/superadmin/assisted_v2/deliver.php" style="display:grid;gap:8px;">
                        <input type="hidden" name="csrf_token" value="<?= sanitize(csrf_token()); ?>">
                        <input type="hidden" name="reqId" value="<?= sanitize($request['reqId'] ?? ''); ?>">
            ...
            <div class="card" style="display:grid;gap:12px;">
                ...
                <form method="post" action="/superadmin/assisted_v2/reject.php" style="display:grid;gap:8px;">
                    <input type="hidden" name="csrf_token" value="<?= sanitize(csrf_token()); ?>">
                    <input type="hidden" name="reqId" value="<?= sanitize($request['reqId'] ?? ''); ?>">
                    <label class="field">

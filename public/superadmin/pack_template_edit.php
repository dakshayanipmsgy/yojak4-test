...
safe_page(function () {
    ...
    $pack = load_pack_template_record($scope, $yojId !== '' ? $yojId : null, $packTemplateId);
    if (!$pack) {
    ...
    render_layout($title, function () use ($pack, $templates, $scope, $yojId, $canAdvanced) {
        ...
        <div class="card" style="display:grid;gap:14px;">
            <div>
                <h2 style="margin:0;">Edit Pack Template (<?= sanitize($scope); ?>)</h2>
        ...
        render_pack_template_editor([
            'pack' => $pack,
            'templates' => $templates,
            'action' => '/superadmin/pack_template_update.php?scope=' . urlencode($scope) . ($yojId ? '&yojId=' . urlencode($yojId) : ''),

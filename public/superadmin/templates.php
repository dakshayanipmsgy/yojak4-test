...
safe_page(function (): void {
    ...
    $templates = load_global_templates_full();
    usort($templates, fn($a, $b) => strcmp($b['updatedAt'] ?? '', $a['updatedAt'] ?? ''));

    $fields = [];
    foreach (array_keys(assisted_v2_canonical_key_set()) as $key) {
        $label = assisted_v2_standard_label_for_key($key) ?: ucwords(str_replace(['.', '_'], ' ', $key));
        $fields[] = ['key' => $key, 'label' => $label];
    }
    foreach (array_keys(pack_tender_placeholder_values([])) as $key) {
        $fields[] = ['key' => $key, 'label' => ucwords(str_replace(['.', '_'], ' ', $key))];
    }
    usort($fields, fn($a, $b) => strcmp($a['label'], $b['label']));

    $title = get_app_config()['appName'] . ' | Global Templates';

    render_layout($title, function () use ($templates, $template, $fields, $actor) {
        ...
        <div class="card" style="display:grid;gap:12px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                ...
                <a class="btn" href="/superadmin/templates.php">Create New</a>
            </div>
                ...
                <?php foreach ($templates as $tpl): ?>
                    <div class="card" style="background:var(--surface-2);display:grid;gap:8px;">
                        <div>
                            ...
                            <p class="muted" style="margin:4px 0 0;"><?= sanitize($tpl['category'] ?? 'tender'); ?></p>
                        </div>
                        <p class="muted" style="margin:0;"><?= sanitize(mb_substr($tpl['description'] ?? $tpl['body'] ?? '', 0, 120)); ?></p>
                        <a class="btn secondary" href="/superadmin/templates.php?templateId=<?= sanitize($tpl['templateId']); ?>">Edit</a>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="card" style="margin-top:16px;display:grid;gap:16px;">
            <div>
                <h3 style="margin:0;"><?= sanitize($template ? 'Edit Template' : 'Create Template'); ?></h3>
                <p class="muted" style="margin:4px 0 0;">Staff can edit with the guidance editor or apply advanced JSON.</p>
            </div>
            <form method="post" action="/superadmin/templates_save.php" id="template-form" style="display:grid;gap:16px;">
                ...
                <div style="display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));align-items:start;">
                    <div style="display:grid;gap:8px;">
                        ...
                        <textarea class="input" name="body" id="template-body" rows="16"><?= sanitize($template['body'] ?? ''); ?></textarea>
                        <p class="muted" style="margin:0;">Use <code>{{field:key}}</code> placeholders. Tokens print as blanks.</p>
                    </div>
                    <div style="display:grid;gap:10px;border:1px solid var(--border);border-radius:12px;padding:12px;background:var(--surface-2);">
                        ...
                        <div style="display:flex;gap:8px;flex-wrap:wrap;">
                            <button class="btn secondary" type="button" data-insert="{{blank:short}}">Blank (Short)</button>
                            <button class="btn secondary" type="button" data-insert="{{blank:long}}">Blank (Long)</button>
                            <button class="btn secondary" type="button" data-insert="{{yesno:Option}}">Yes/No</button>
                        </div>
                ...
                <div style="border:1px solid var(--border);border-radius:12px;padding:12px;display:grid;gap:10px;">
                    ...
                    <p class="muted" style="margin:0;">Add tables and insert <code>{{field:table:&lt;tableId&gt;}}</code>.</p>
            ...
            <div style="border:1px solid var(--border);border-radius:12px;padding:12px;display:grid;gap:12px;background:var(--surface-2);">
                ...
                <form method="post" action="/superadmin/templates_json_validate.php" style="display:grid;gap:10px;">
                    <input type="hidden" name="csrf_token" value="<?= sanitize(csrf_token()); ?>">
                    ...
                    <div style="display:flex;gap:8px;flex-wrap:wrap;">
                        ...
                        <button class="btn" type="submit" formaction="/superadmin/templates_json_apply.php">Apply JSON</button>
                    </div>
                </form>
            </div>
        </div>
